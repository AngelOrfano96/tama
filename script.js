...
const decayRates = { hunger: 0.02, fun: 0.01, clean: 0.01 }; // Modifica valori per velocità desiderata
const SAVE_INTERVAL_SEC = 5; // ogni quanti secondi salvare (cambia solo qui!)
...
function startLiveTick() {
  ...
  saveInterval = setInterval(saveState, SAVE_INTERVAL_SEC * 1000);
}
...
async function initFlow() {
  ...
  if (state) {
    ...
    // Qui la correzione: applica solo se è passato "davvero" del tempo!
    if (elapsed > 2 * SAVE_INTERVAL_SEC) {
